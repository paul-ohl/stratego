...
async function move(board, originId, destinationId) {
	...
	// 	alert("Error!");
	// }
	[board[originId], board[destinationId]] = [board[destinationId], board[originId]]
	drawBoard(board);
...
function drawPossibleMoves(board, idx) {
	...
	for (cellId of surroundingCells) {
		...
		board[cellId].addEventListener("click", () => { move(board, origin, destination) });
...
function drawPlayableCells(board) {
	for ([idx, cell] of board.entries()) {
		...
		if (surroundingCells.some(nodeId => !board[nodeId].classList.contains('player-cell'))) {
